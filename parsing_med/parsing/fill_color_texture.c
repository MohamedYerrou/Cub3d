/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   fill_color_texture.c                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: myerrou <myerrou@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/19 08:57:27 by myerrou           #+#    #+#             */
/*   Updated: 2025/02/19 09:18:05 by myerrou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../cub3d.h"

static bool	no_digit(char *str)
{
	int		i;
	bool	found_no_digit;

	i = 0;
	found_no_digit = true;
	while (str[i])
	{
		if (ft_isdigit(str[i]) == 1)
			found_no_digit = false;
		i++;
	}
	return (found_no_digit);
}

static int	*into_array(char **rgb_to_convert, int *rgb)
{
	int		i;

	i = -1;
	while (rgb_to_convert[++i])
	{
		rgb[i] = ft_atoi(rgb_to_convert[i]);
		if (rgb[i] == -1 || no_digit(rgb_to_convert[i]) == true)
		{
			free_tab((void **)rgb_to_convert);
			free(rgb);
			return (0);
		}
	}
	free_tab((void **)rgb_to_convert);
	return (rgb);
}

static int	*set_rgb_colors(char *line)
{
	char	**rgb_to_convert;
	int		*rgb;
	int		count;

	rgb_to_convert = ft_split(line, ',');
	count = 0;
	while (rgb_to_convert[count])
		count++;
	if (count != 3)
	{
		free_tab((void **)rgb_to_convert);
		return (0);
	}
	rgb = malloc(sizeof(int) * 3);
	if (!rgb)
	{
		message(NULL, "Error in malloc", 0);
		return (0);
	}
	return (into_array(rgb_to_convert, rgb));
}

int	fill_color_textures(t_data *data, t_texdetail *textures, char *line, int j)
{
	if (line[j + 1] && ft_isprint(line[j + 1]))
		return (message(data->mapdet.path, "Invalid colors", ERR));
	if (!textures->roof && line[j] == 'C')
	{
		textures->roof = set_rgb_colors(line + j + 1);
		if (textures->roof == 0)
			return (message(data->mapdet.path, "color invalid in roof", ERR));
	}
	else if (!textures->floor && line[j] == 'F')
	{
		textures->floor = set_rgb_colors(line + j + 1);
		if (textures->floor == 0)
			return (message(data->mapdet.path, "color invalid in floor", ERR));
	}
	else
		return (message(data->mapdet.path, "Invalid colors", ERR));
	return (SUCCESS);
}
