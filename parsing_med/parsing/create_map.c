...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/19 08:57:22 by myerrou           #+#    #+#             */
/*   Updated: 2025/02/19 09:18:57 by myerrou          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	count_map_lines(t_data *data, char **file, int i)
{
	...
	data->mapdet.index_end_of_map = i;
...
static int	fill_map_tab(t_data *data, char **map_tab, int index)
{
	...
	data->w_map = max_len(data->mapdet, index);
	i = 0;
	while (i < data->h_map)
	{
		j = 0;
		...
		while (data->mapdet.file[index][j]
			&& data->mapdet.file[index][j] != '\n')
		{
			map_tab[i][j] = data->mapdet.file[index][j];
			j++;
		}
		while (j < data->w_map)
			map_tab[i][j++] = '\0';
