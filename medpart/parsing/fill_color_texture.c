...
int	fill_color_textures(t_data *data, t_texdetail *textures, char *line, int j)
{
	if (line[j + 1] && ft_isprint(line[j + 1]))
		return (message(data->mapdetail.path, "Invalid colors", ERR));
	if (!textures->roof && line[j] == 'C')
	{
		textures->roof = set_rgb_colors(line + j + 1);
		if (textures->roof == 0)
			return (message(data->mapdetail.path, "color invalid in roof", ERR));
	}
	else if (!textures->floor && line[j] == 'F')
	{
		textures->floor = set_rgb_colors(line + j + 1);
		if (textures->floor == 0)
			return (message(data->mapdetail.path, "color invalid in floor", ERR));
	}
	else
		return (message(data->mapdetail.path, "Invalid colors", ERR));
	return (SUCCESS);
